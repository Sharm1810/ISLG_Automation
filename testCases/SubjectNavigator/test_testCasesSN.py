...
class Test_testCasesSN:
    ...
    def test_Branches(self):
        self.logger.info("****TestCase 001 - Test Branches***")
        ...
        time.sleep(2)
        links = self.driver.find_elements(By.XPATH, "//div[@class='item-list compact__container']//div//a")
        # traverse list
        for lnk in links:
            # get_attribute() to get all href
            print(lnk.get_attribute('title'))
            # prints all the link title A-Z
            self.logger.info(lnk.get_attribute('title'))
            time.sleep(2)
        for lnk in links:
            # clicks on the links (Expand)
            lnk.click()
            time.sleep(3)
            self.driver.save_screenshot(".\\Screenshots\\" + "ExpandBranches.png")
            time.sleep(2)
        for lnk in links:
            # clicks on the links (Collapse)
            lnk.click()
            time.sleep(3)
            self.driver.save_screenshot(".\\Screenshots\\" + "ExpandAndCollapseBranches.png")
            time.sleep(2)

    @pytest.mark.skip(reason="None")
    def test_addtoResearch(self):
        self.logger.info("****TestCase 002 - Verify Add to Research***")
        self.navigator = SubjectNavigator(self.driver)
        self.navigator.clickOnSubjectNavigatormenu()
        self.logger.info("Clicked on Subject Navigator from the navigation menu")
        ...
        try:
            self.navigator.clickOnActionsDropdown()
            self.logger.info("Clicked on Actions")
            time.sleep(2)
            self.navigator.clickOnResearchNotepadDropdown()
            time.sleep(2)
            self.navigator.selectResearchOption()
            time.sleep(2)
            self.navigator.clickOnAddNotepad()
            time.sleep(2)
            error = self.navigator.checkBookMarkError()
            if error == "Entry already exists in this topic.":
                print("Already added to Notepad")
                self.logger.info("Already exists in this topic")
    ...
    def test_copyLocation(self):
        self.logger.info("****TestCase 003 - Copy Location***")
        ...
        try:
            ...
            self.logger.info("Clicked on Actions")
            time.sleep(2)
            self.navigator.clickOnCopyLocationDropdown()
            time.sleep(2)
            toastMessage = self.navigator.checkToastMessage()
            time.sleep(5)
            if toastMessage == "Location copied to your clipboard.":
                self.logger.info(toastMessage)
            else:
                self.logger.info("No message is displayed")
        except NoSuchElementException:
            print("No such element")
        # element = self.driver.find_element(By.XPATH, "//input[@id='search-subject]")
        time.sleep(2)
